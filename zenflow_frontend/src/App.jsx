            ...
            <AnimatePresence mode="wait">
              <Routes location={location} key={location.pathname}>
                {/* Dynamic Routing Structure:
                  All app routes are now prefixed with /:username 
                */}
                <Route path="/:username" element={<Dashboard user={user} backendUrl={BACKEND_URL} />} />
                <Route path="/:username/practice" element={<Practice user={user} backendUrl={BACKEND_URL} />} />
                <Route path="/:username/journal" element={<Journal user={user} backendUrl={BACKEND_URL} />} />
                <Route path="/:username/coach" element={<Coach user={user} backendUrl={BACKEND_URL} />} />
                <Route path="/:username/library" element={<Library user={user} backendUrl={BACKEND_URL} />} />
...
function AuthScreen({ onLogin }) {
  ...
  const poseVariants = {
      center: { x: 0, scale: 1.15, opacity: 1, zIndex: 20, filter: "drop-shadow(0px 30px 50px rgba(168, 85, 247, 0.8))" },
      left: { x: "-45%", scale: 0.75, opacity: 0.3, zIndex: 10, filter: "drop-shadow(0px 0px 0px rgba(168, 85, 247, 0))" },
      right: { x: "45%", scale: 0.75, opacity: 0.3, zIndex: 10, filter: "drop-shadow(0px 0px 0px rgba(168, 85, 247, 0))" }
  };
  ...
  return (
    <div className="min-h-screen relative flex items-center justify-center p-4 md:p-8 overflow-hidden w-full">
      ...
      <div className="max-w-6xl w-full mx-auto relative z-10 flex flex-col md:flex-row items-center justify-between gap-12 lg:gap-20">
        ...
        >
            <motion.div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-white/5 border border-white/10 text-xs font-medium text-purple-300 mb-8">
            ...
            <form className="w-full flex flex-col gap-4" onSubmit={handleAuthAction}>
                <div className="relative">
                    ...
                    <input 
                        ...
                        className="w-full bg-white/[0.03] border border-white/10 rounded-xl py-4 pl-12 pr-4 text-white focus:outline-none focus:border-purple-500/50 transition-all"
                        required
                    />
                </div>
                <div className="relative">
                ...
                {error && <div className="p-4 mt-2 rounded-xl bg-rose-500/10 text-rose-400 text-xs text-center border border-rose-500/20">{error}</div>}
                <motion.button
                    type="submit"
                    disabled={isSubmitting}
                    whileHover={{ scale: 1.02 }}
                    whileTap={{ scale: 0.98 }}
                    className="w-full mt-4 group relative px-8 py-4 rounded-xl bg-white text-black font-semibold flex items-center justify-center gap-2 shadow-lg transition-all disabled:opacity-60"
                >
                    <span className="relative z-10">{isSubmitting ? "Please wait..." : isRegistering ? "Commence Journey" : "Re-enter Sanctuary"}</span>
                    <ArrowRight className="w-4 h-4 group-hover:translate-x-1 transition-transform" />
                </motion.button>
            </form>
            <p className="mt-8 text-sm text-gray-400 self-center">
                {isRegistering ? "Already a practitioner?" : "New seeker?"}{" "}
                <button type="button" onClick={() => setIsRegistering(!isRegistering)} className="text-purple-400 hover:text-purple-300 transition-colors">
        ...
        >
            <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
                <motion.div animate={{ scale: [1, 1.2, 1], opacity: [0.3, 0.6, 0.3] }} transition={{ duration: 4, repeat: Infinity }} className="w-64 h-64 md:w-[450px] md:h-[450px] bg-purple-600/40 rounded-full blur-[80px]" />
            </div>
            <div className="relative w-full h-full flex items-center justify-center">
                {poses.map((pose, index) => (
                    ...
                    >
                        <img src={pose} alt={`Pose ${index + 1}`} className="w-full h-full object-contain" />
                    </motion.div>
                ))}
            </div>
        </motion.div>
